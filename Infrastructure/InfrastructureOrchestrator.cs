    ...
    public async Task<InfrastructureDeploymentOutputs> DeployAsync(InfrastructureSettings settings, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!preDeploymentValidation.IsValid)
                throw new ConfigurationValidationException($"Pre-deployment validation failed with {preDeploymentValidation.Errors.Count} errors. Please fix the configuration issues before deployment.", "PreDeploymentValidation", "ConfigurationValidation");
